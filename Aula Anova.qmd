...
Usamos lme4, para checar pelo modelo misto, porque Em parcelas subdivididas não é possível checar as premissas pelo check\_.

```{r}
library(lme4)
names(milho)
milho$block <- as.factor(milho$tipo)
mix2 <- lmer(tipo ~ block + umidade*tempo +
(1|block/umidade), data =  milho)
...
```{r}
means_mix2 <- emmeans(mix2, ~tipo | umidade)
means_mix2
```
